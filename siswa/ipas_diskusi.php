...
<body>
  ...
  <div class="main-content">
    <header>
      <div class="hamburger-logo">
        <button class="hamburger" id="sidebarToggleBtn" onclick="toggleSidebar()">
          <svg id="sidebarArrow" viewBox="0 0 24 24" fill="none"><path d="M8 5l8 7-8 7" stroke="#009FFD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
      </div>
      <div class="profile-menu">
        ...
        <div class="dropdown" id="dropdown">
          <a href="siswa_edit_profile.php"><svg width="18" height="18" fill="none" stroke="#009FFD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19.5 3 21l1.5-4L16.5 3.5z"/></svg>Edit Profil</a>
          <a href="../logout.php"><svg width="18" height="18" fill="none" stroke="#009FFD" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="M16 17l5-5-5-5"/><path d="M21 12H9"/></svg>Logout</a>
        </div>
      </div>
    </header>
    <div class="diskusi-container">
      <div class="diskusi-header">
        <div class="diskusi-icon">
          <svg fill="none" stroke="#fff" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2z"/></svg>
        </div>
        <div>
          <h1 class="diskusi-title">Forum Diskusi IPAS</h1>
          <p class="diskusi-subtitle">Diskusikan materi pelajaran dengan teman dan guru</p>
        </div>
      </div>
    </div>
    <!-- CHAT BOX START -->
    <div class="chatbox-container" style="margin-top:2.5rem;">
      ...
      <div class="chatbox-panel" id="chatPanelPribadi" style="display:none;">
        <div style="margin-bottom:0.7rem;">
          <input type="text" class="chatbox-user-input" id="chatTargetUser" value="guru" readonly style="width:220px;background:#e6f7ff;color:#288246ff;font-weight:700;" />
        </div>
        <div class="chatbox-messages" id="chatMessagesPribadi"></div>
        <form class="chatbox-form" id="chatFormPribadi" autocomplete="off">
          <input type="text" class="chatbox-input" id="chatInputPribadi" placeholder="Ketik pesan pribadi ke guru..." maxlength="300" required />
          <button type="submit" class="chatbox-send">Kirim</button>
        </form>
      </div>
    </div>
    <style>
      .chatbox-container {
        background: rgba(255,255,255,0.13);
        border-radius: 18px;
        box-shadow: 0 4px 24px #288246ff;
        max-width: 600px;
      ...
      .chatbox-tabs {
        display: flex;
        border-bottom: 2px solid #288246ff;
        margin-bottom: 0.5rem;
      }
      .chatbox-tab {
        flex: 1;
      ...
      .chatbox-message .chatbox-text {
        ...
        font-size: 1rem;
        box-shadow: 0 2px 8px #288246ff;
      }
      .chatbox-message.me .chatbox-text {
        background: #288246ff;
        color: #fff;
        border-radius: 14px 14px 0 14px;
        box-shadow: 0 2px 8px #2a2a7255;
      }
      .chatbox-form {
        display: flex;
        gap: 0.7rem;
  ...
  <script>
    ...
    function updateSidebarArrow() {
      ...
      if (sidebar.classList.contains('open')) {
        arrow.innerHTML = '<path d="M16 5l-8 7 8 7" stroke="#288246ff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
      } else {
        arrow.innerHTML = '<path d="M8 5l8 7-8 7" stroke="#288246ff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
      }
    }
    document.addEventListener('DOMContentLoaded', function() {
      ...
      <?php endif; ?>
      let lastMsgsGrup = [];
      // FUNGSI CHAT NORMAL, TIDAK HAPUS OTOMATIS SETIAP RELOAD
  ...
  <div id="deleteModal" style="display:none;position:fixed;z-index:9999;left:0;top:0;width:100vw;height:100vh;background:rgba(0,0,0,0.25);align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:18px;box-shadow:0 8px 32px #009ffd55;padding:2.2rem 2.5rem;max-width:90vw;min-width:320px;text-align:center;position:relative;">
      <div style="color:#222;font-size:1.1rem;margin:1.1rem 0 2.1rem 0;">Hapus aja nih?</div>
      <button id="deleteModalCancel" style="background:#eee;color:#009FFD;font-weight:600;border:none;border-radius:8px;padding:0.7rem 1.5rem;font-size:1rem;cursor:pointer;margin-right:1.2rem;">Batal</button>
      <button id="deleteModalOk" style="background:#009FFD;color:#fff;font-weight:700;border:none;border-radius:8px;padding:0.7rem 1.5rem;font-size:1rem;cursor:pointer;">Hapus</button>
    </div>
  </div>
  <script>
    // Modal hapus pesan

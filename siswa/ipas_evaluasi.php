...
function sudah_upload($eval_id, $siswa_id, $submissions) {
    foreach ($submissions as $sub) {
        if ($sub['evaluasi_id'] == $eval_id && $sub['siswa_id'] == $siswa_id) {
            return $sub;
        }
    }
    return false;
...
<head>
    <meta charset="UTF-8">
    <title>Evaluasi IPAS Siswa - CUAN</title>
    ...
    .eval-card-list { display: grid; grid-template-columns: repeat(auto-fit, minmax(420px, 1fr)); gap: 2.2rem; margin-bottom: 2.2rem; align-items: stretch; }
    .eval-card { background: linear-gradient(135deg, #fffbe6 60%, #f5e8d8 100%); border-radius: 22px; padding: 1.6rem 1.4rem 1.2rem 1.4rem; box-shadow: 0 6px 32px 0 rgba(139,94,60,0.13), 0 1.5px 8px 0 rgba(139,94,60,0.08); display: flex; flex-direction: column; gap: 1.1rem; transition: transform 0.32s cubic-bezier(.68,-0.55,.27,1.55), box-shadow 0.32s; animation: fadeIn 0.6s ease-out; border-left: 7px solid var(--eval-primary); position: relative; overflow: hidden; min-width: 0; width: 100%; max-width: 100%; margin-bottom: 0; }
    .eval-card-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 12px; z-index: 1; position: relative; }
    .eval-card-title { font-size: 1.45rem; font-weight: 800; color: var(--eval-primary); margin: 0; letter-spacing: 0.5px; text-shadow: 0 1.5px 8px rgba(139,94,60,0.07); }
    .eval-card-date { font-size: 1rem; color: #fff; background: linear-gradient(90deg, var(--eval-primary) 60%, var(--eval-accent) 100%); padding: 0.32rem 1.1rem; border-radius: 18px; font-weight: 600; box-shadow: 0 1.5px 8px rgba(139,94,60,0.08); border: none; }
    .eval-card-desc { font-size: 1.08rem; color: #3a2b1a; line-height: 1.7; margin-bottom: 0.2rem; z-index: 1; position: relative; }
    .eval-card-submissions { background: linear-gradient(90deg, #fff 60%, #f5e8d8 100%); border-radius: 14px; padding: 1.1rem 1rem 0.7rem 1rem; border: 1.5px dashed var(--eval-accent); box-shadow: 0 1.5px 8px rgba(139,94,60,0.04); z-index: 1; position: relative; }
    .eval-submissions-title { font-weight: 800; color: var(--eval-primary); margin: 0 0 0.7rem 0; font-size: 1.08rem; letter-spacing: 0.2px; }
    .eval-submissions { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 14px; }
    .eval-submissions li { background: linear-gradient(90deg, #f9e7d1 60%, #fff 100%); border: 1.5px solid var(--eval-accent); border-radius: 13px; padding: 13px 16px; display: flex; flex-wrap: wrap; align-items: center; gap: 12px; justify-content: space-between; box-shadow: 0 1.5px 8px rgba(139,94,60,0.06); position: relative; transition: box-shadow 0.2s, transform 0.2s; }
    .eval-submissions li:hover { box-shadow: 0 6px 24px rgba(139,94,60,0.13); transform: scale(1.015) translateY(-2px); }
    .eval-sub-left { display: flex; flex-direction: column; gap: 5px; }
    .eval-sub-meta { font-size: 1.01rem; color: #4b3a2a; font-weight: 500; letter-spacing: 0.1px; }
    .eval-sub-actions form { display: inline-flex; gap: 10px; align-items: center; }
    .eval-grade-input { width: 90px; padding: 7px 10px; border-radius: 8px; border: 1.5px solid var(--eval-primary); font-size: 1rem; background: #fffbe6; font-weight: 600; color: var(--eval-primary); outline: none; transition: border 0.2s; }
    .eval-grade-input:focus { border: 1.5px solid var(--eval-primary-dark); background: #fff9e0; }
    .eval-btn { background: linear-gradient(90deg, var(--eval-primary) 60%, var(--eval-accent) 100%); border: none; color: #fff; padding: 0.5rem 1.1rem; border-radius: 9px; font-size: 1rem; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 0.4rem; transition: background 0.3s, transform 0.2s, box-shadow 0.2s; box-shadow: 0 1.5px 8px rgba(139,94,60,0.08); letter-spacing: 0.2px; }
    .eval-btn:hover { background: linear-gradient(90deg, var(--eval-primary-dark) 60%, var(--eval-accent) 100%); transform: scale(1.06); box-shadow: 0 4px 16px rgba(139,94,60,0.13); }
    .eval-form-card { background: linear-gradient(135deg, #fffbe6 60%, #f5e8d8 100%); border-radius: 22px; padding: 1.6rem 1.4rem 1.2rem 1.4rem; box-shadow: 0 6px 32px 0 rgba(139,94,60,0.13), 0 1.5px 8px 0 rgba(139,94,60,0.08); border-left: 7px solid var(--eval-primary); animation: fadeIn 0.6s ease-out; margin-bottom: 2.2rem; position: relative; overflow: hidden; }
    @media screen and (max-width: 1024px) {
      .eval-container { padding: 1.1rem; }
      .eval-card-list { grid-template-columns: repeat(auto-fit, minmax(340px, 1fr)); gap: 1.2rem; }
    ...
    @media screen and (max-width: 768px) {
      ...
      .eval-card-list { grid-template-columns: 1fr; gap: 0.7rem; }
    ...
    @media screen and (max-width: 480px) {
      .eval-card-list { grid-template-columns: 1fr; gap: 0.3rem; }
    ...
    <div class="eval-container">
      <div class="eval-header">
        ...
        <div>
          ...
          <p class="eval-subtitle">Kumpulkan evaluasi yang diberikan guru dan lihat hasil penilaian</p>
        </div>
      </div>
      <div class="eval-section-title">Daftar Evaluasi & Pengumpulan Siswa</div>
      <div class="eval-card-list">
        <?php if (empty($evaluasi)): ?>
          <div class="eval-card eval-empty" style="background:rgba(255,255,255,0.1);border-radius:16px;padding:3rem 2rem;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,0.1);animation:fadeIn 0.8s ease-out;">
            <svg width="80" height="80" style="margin-bottom:1.5rem;opacity:0.7;" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="10" y="8" width="60" height="60" rx="12" fill="#fffbe6" stroke="#C49A6C" stroke-width="3"/><path d="M30 38h28M30 50h18" stroke="#C49A6C" stroke-width="3" stroke-linecap="round"/></svg>
            <div class="eval-empty-text" style="font-size:1.4rem;font-weight:600;color:#8B5E3C;margin-bottom:0.5rem;">Belum ada evaluasi IPAS dari guru</div>
            <div class="eval-empty-subtext" style="font-size:1rem;color:rgba(139,94,60,0.7);max-width:500px;margin:0 auto;">Evaluasi akan muncul di sini jika guru sudah mengunggah evaluasi baru. Silakan cek secara berkala.</div>
          </div>
        <?php endif; ?>
        <?php foreach ($evaluasi as $e): ?>
          <div class="eval-card">
            ...
            <?php endif; ?>
            <?php
  $upload = sudah_upload($e['id'], $siswa_id, $submissions);
  if (!$upload) {
    $legacy_id = null;
    if (is_array($siswa_list) && isset($siswa_list['status']) && $siswa_list['status'] === 'success' && isset($siswa_list['data'])) {
    ...
    if ($legacy_id !== null) {
      $upload = sudah_upload($e['id'], $legacy_id, $submissions);
    }
  }
?>
            <div class="eval-card-submissions">
              ...
              <?php
              // Tampilkan semua pengumpulan siswa untuk evaluasi ini
              foreach ($submissions as $sub) {
                if ($sub['evaluasi_id'] == $e['id']) {
                  $sid = $sub['siswa_id'];
                  $username = isset($siswa_map[$sid]) ? $siswa_map[$sid] : 'Siswa: '.$sid;
                  ...
                  echo '<div class="eval-sub-meta" style="margin-bottom:6px;">';
                  echo '<b style="color:var(--eval-primary);font-size:1.08em;">'.$username.'</b>';
                  echo ' <span style="color:#8B5E3C;font-size:0.98em;">('.$upload_time_local.')</span>';
                  echo '</div>';
                  if (!empty($sub['file'])) {
                  ...
                  echo '<hr style="margin:10px 0;border:0;border-top:1px dashed #C49A6C;">';
                }
              }
              ?>
              <?php if (!$upload): ?>
                <form class="form-upload-evaluasi" action="upload_evaluasi.php" method="post" enctype="multipart/form-data">
                  ...
                  <button type="submit" class="eval-btn">Kumpulkan</button>
                </form>
              <?php endif; ?>
            </div>
          </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      document.querySelectorAll('.form-upload-evaluasi').forEach(function(form) {
      ...
      if (sessionStorage.getItem('popup_upload_success_eval')) {
        showCustomPopupEval('Evaluasi berhasil dikumpulkan! 🎉', { confirm: false });

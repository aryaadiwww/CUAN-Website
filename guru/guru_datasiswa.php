...
<body>
  ...
  <style>
    .students-container {
      ...
      flex-direction: column;
      align-items: stretch;
      animation: fadeInStudents 1.1s cubic-bezier(.68,-0.55,.27,1.55);
    }
    @keyframes fadeInStudents {
    ...
    .students-title {
      ...
      text-shadow: 0 2px 8px #b8355633;
      text-align: center;
      animation: pulseTitle 1.2s cubic-bezier(.68,-0.55,.27,1.55);
      margin-right: 6rem;
    }
    ...
    .student-card {
      background: linear-gradient(135deg, #f7e6e9 60%, #e4f0fa 100%);
      border-radius: 12px;
      box-shadow: 0 1px 6px #b8355633;
      display: flex;
      align-items: center;
      gap: 0.7rem;
    ...
    .student-card:hover {
      ...
      box-shadow: 0 8px 32px #b8355655;
      background: linear-gradient(135deg, #ffe6f0 60%, #d6eaff 100%);
    }
    @keyframes cardPop {
    ...
    .student-icon {
      ...
      height: 38px;
      border-radius: 50%;
      background: linear-gradient(135deg, #b83556 40%, #f9a826 100%);
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 1.2rem;
      ...
      animation: iconBounce 1.1s cubic-bezier(.68,-0.55,.27,1.55);
      flex-shrink: 0;
    }
    ...
    .detail-btn {
      background: linear-gradient(90deg, #b83556 0%, #f9a826 100%);
      color: #fff;
      border: none;
      border-radius: 50px;
      ...
      align-items: center;
      gap: 0.4rem;
      animation: btnFadeIn 1.2s cubic-bezier(.68,-0.55,.27,1.55);
      flex-shrink: 0;
    }
    .detail-btn:hover {
  ...
  <div class="students-container">
    <div class="students-title">Data Siswa</div>
  ...
  <script>
    ...
    function renderStudents() {
      fetch('api_guru_datasiswa.php')
        .then(response => response.json())
        .then(students => {
          const list = document.getElementById('students-list');
          list.innerHTML = '';
          const profileIcon = '<img src="../img/profile.png" alt="Profile" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">';
          students.forEach((student, i) => {
            const card = document.createElement('div');
            card.className = 'student-card';
            card.innerHTML = `
              <div class="student-icon" style="background: linear-gradient(135deg, #${(i%2===0)?'b83556':'f9a826'} 40%, #${(i%2===0)?'f9a826':'b83556'} 100%); animation-delay: ${i*0.04}s;">
                ${profileIcon}
              </div>
              <div class="student-name">${student.nama}</div>
              <button class="detail-btn" type="button" data-username="${encodeURIComponent(student.username)}">
                <svg width="18" height="18" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
                Detail
              </button>
            `;
    ...
    // Fungsi detail siswa, gunakan username
    function showDetail(username) {
      location.href = 'detailsiswa.php?username=' + encodeURIComponent(username);
    }

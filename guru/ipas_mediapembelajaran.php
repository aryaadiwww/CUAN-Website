...
<head>
  ...
  .bahanajar-header {
    ...
    .bahanajar-card {
      ...
      border-radius: 16px;
      padding: 1.5rem;
      box-shadow: 0 4px 20px rgba(191, 168, 0, 0.13);
      display: flex;
      flex-direction: column;
      gap: 1rem;
      transition: transform 0.3s, box-shadow 0.3s;
      animation: fadeIn 0.8s ease-out;
      border-left: 4px solid #bfa800;
    }
    ...
    .bahanajar-card-date {
      font-size: 0.9rem;
      color: #bfa800;
      background: rgba(191, 168, 0, 0.13);
...
<body>
  ...
  <div class="main-content">
    ...
    <div class="bahanajar-container">
      <div class="bahanajar-header">
        ...
        <div>
          <h1 class="bahanajar-title">Media Pembelajaran IPAS</h1>
          <p class="bahanajar-subtitle">Kelola media pembelajaran untuk siswa</p>
        </div>
      </div>
  ...
  <!-- Tombol tambah media pembelajaran dan modal upload -->
  <button class="bahanajar-add-button" onclick="showUploadModal()">
          ...
          Tambah Media Pembelajaran Baru
        </button>
        ...
        <div id="uploadModal" style="display:none;position:fixed;z-index:2000;left:0;top:0;width:100vw;height:100vh;background:rgba(0,0,0,0.4);align-items:center;justify-content:center;">
          <div style="background:#fff;padding:2rem 2rem 1rem 2rem;border-radius:16px;max-width:400px;width:90vw;position:relative;">
            <h2 style="margin-top:0;color:#ffd700;">Upload Media Pembelajaran</h2>
            <form id="uploadForm">
              ...
              <button type="submit" style="background:#ffd700;color:#222;padding:0.5rem 1.5rem;border:none;border-radius:8px;font-weight:600;">Upload</button>
  ...
  <!-- Tempat render media pembelajaran -->
  ...
  <script>
    ...
    document.getElementById('uploadForm').onsubmit = function(e) {
      ...
      fetch('upload_media_pembelajaran.php', {
        method: 'POST',
        body: formData
      })
      .then(r => r.json())
  ...
  // Render media pembelajaran
  function mediaPembelajaranCard(entry) {
        ...
        <div class="bahanajar-card-footer">
          <div class="bahanajar-card-info">
            <div class="bahanajar-card-stats" style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;">
              ${entry.filename ? `<span style="margin-bottom:2px;">File: <a href="../uploads/media_pembelajaran/${entry.filename}" target="_blank" style="color:#bfa800;text-decoration:underline;word-break:break-all;">${entry.original}</a></span>` : ''}
              ${entry.link ? `<span style="margin-bottom:2px;">Link: <a href="${entry.link}" target="_blank" style="color:#bfa800;text-decoration:underline;word-break:break-all;">${entry.link}</a></span>` : ''}
            </div>
            <div class="bahanajar-card-time"></div>
          </div>
          <div class="bahanajar-card-actions" style="position:relative;">
            ${entry.filename ? `<a class="bahanajar-card-button" href="../uploads/media_pembelajaran/${entry.filename}" download>Unduh</a>
            <a class="bahanajar-card-button" href="../uploads/media_pembelajaran/${entry.filename}" target="_blank">Lihat</a>` : ''}
            ${entry.link ? `<a class="bahanajar-card-button" href="${entry.link}" target="_blank">Kunjungi Link</a>` : ''}
            <button class="bahanajar-card-button" style="background:#c00;color:#fff;" onclick="hapusMediaPembelajaran(this, '${entry.filename || ''}', '${entry.link || ''}', '${entry.title.replace(/'/g, "\'")}')">Hapus</button>
          </div>
        </div>
      </div>`;
    }
    function loadMediaPembelajaran() {
      fetch('../uploads/media_pembelajaran/media_pembelajaran.json?' + Date.now())
        .then(r => r.json())
        .then(data => {
          const list = document.getElementById('bahanajar-list');
          if (!data || !data.length) {
            list.innerHTML = `<div class='bahanajar-empty'><h2 class='bahanajar-empty-text'>Belum Ada Media Pembelajaran</h2><p class='bahanajar-empty-subtext'>Anda belum menambahkan media pembelajaran untuk mata pelajaran ini. Klik tombol di atas untuk menambahkan media pembelajaran baru.</p></div>`;
          } else {
            list.innerHTML = data.map(mediaPembelajaranCard).join('');
          }
        })
        .catch(() => {
          document.getElementById('bahanajar-list').innerHTML = `<div class='bahanajar-empty'><h2 class='bahanajar-empty-text'>Belum Ada Media Pembelajaran</h2><p class='bahanajar-empty-subtext'>Gagal memuat data media pembelajaran.</p></div>`;
        });
    }
    document.addEventListener('DOMContentLoaded', function() {
      loadMediaPembelajaran();
    });
    // Fungsi hapus media pembelajaran
    function hapusMediaPembelajaran(btn, filename, link, title) {
      if (!confirm('Yakin ingin menghapus media pembelajaran ini?')) return;
      btn.disabled = true;
      btn.innerText = 'Menghapus...';
      fetch('delete_media_pembelajaran.php', {
        method: 'POST',
      ...
      .then(data => {
        if (data.success) {
          ...
          alert(data.error || 'Gagal menghapus media pembelajaran');
          btn.disabled = false;
          btn.innerText = 'Hapus';
        }
      })
      .catch(() => {
        alert('Gagal menghapus media pembelajaran');
        btn.disabled = false;
        btn.innerText = 'Hapus';
      });
    }
